...
class PaidLoansPage extends StatelessWidget {
  ...
  PaidLoansPage({Key? key}) : super(key: key) {
    scrollController.addListener(() {
      if (scrollController.position.pixels == scrollController.position.maxScrollExtent &&
          !loanController.isLoading.value &&
          loanController.currentPaidPage.value < loanController.totalPages.value) {
        loanController.fetchPaidLoans(page: loanController.currentPaidPage.value + 1);
      }
    });
  }
  ...
  Widget buildLoanList() {
    return ListView.builder(
      ...
      itemCount: loanController.paidLoans.length,
      itemBuilder: (context, index) {
        final loan = loanController.paidLoans[index];
        return Card(
      ...
      body: Padding(
        ...
        child: Column(
          children: <Widget>[
            ...
            //           loanController.fetchPaidLoans(page: 1);
            //         },
            //       ),
            //     ),
            ...
            Expanded(
              child: Obx(() {
                if (loanController.isLoading.value && loanController.paidLoans.isEmpty) {
                  return const Center(child: CircularProgressIndicator());
                } else if (loanController.paidLoans.isEmpty) {
                  return const Center(child: Text('No paid loans found.'));
                } else {
            ...
            Obx(() {
              if (loanController.isLoading.value && loanController.paidLoans.isNotEmpty) {
